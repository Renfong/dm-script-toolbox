...
Image FitAndShowStack(Image src, Image fitrange, number cen, Image &pars){
	Image FitResult := FitErfFunc(src, fitrange, cen, pars)

	Image stack := NewImage("Fitting stack", 2, 250, 3)
	stack[icol,0] = src
	stack[icol,1] = FitResult
	stack[icol,2] = src - FitResult
...
Image lfFit := FitAndShowStack(left, fitrange,50, pars)
result("  left [cen, sig] = ["+GetPixel(pars,1,0)+","+GetPixel(pars,0,0)+"]\n")

Image rtFit := FitAndShowStack(right, fitrange,50, pars)
result(" right [cen, sig] = ["+GetPixel(pars,1,0)+","+GetPixel(pars,0,0)+"]\n")
